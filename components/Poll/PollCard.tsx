...
export default function PollCard({ poll }: { poll: IPoll }) {
  ...
  return (
    <div className="rounded-xl p-4 shadow-[0px_0px_16px_0px_#00000029]">
      ...
      <h3 className="text-gray-900 font-semibold mb-2">{poll.title}</h3>
      ...
      {poll.description && (
        <>
          ...
          <button className="text-gray-700 font-medium text-xs mb-3">
            Read more
          </button>
        </>
      )}
      ...
      <button className="w-full py-2.5 bg-gray-200 text-gray-900 font-semibold rounded-lg">
        Vote
      </button>
    </div>
  );
